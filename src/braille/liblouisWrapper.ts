// Liblouis WebAssembly build copied from the native-liblouis npm package.

// eslint-disable-next-line @typescript-eslint/ban-ts-comment
// @ts-expect-error
import LibLouisFactory from '/src/braille/liblouis.js';
 ...
 */
export const lou_initialize = async (): Promise<boolean> => {
 ...
 */
export const lou_translateString = (text: string, tables: string): string => {
    ensureInitialized();
 ...
 * @param tables Comma-separated table names
 */
export const lou_backTranslateString = (dots: string, tables: string): string => {
    ensureInitialized();
    const tablePath = resolveTablePath(tables);
    return runTranslation(dots, tablePath, 'lou_backTranslateString');
};

export const lou_isInitialized = (): boolean => {
    return initialized;
}
